...
import Tool from "@/components/ImageCompressor";

const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://miguelacm.es/tools/image-compressor";
const EMBED_URL = process.env.NEXT_PUBLIC_EMBED_URL || "https://miguelacm.es/embed/image-compressor";
      ...
      <main className="min-h-screen px-4 py-12">
        <div className="mx-auto max-w-4xl">
          <div className="mb-10 text-center">
            ...
            <h1 className="mb-3 text-4xl font-bold text-white md:text-5xl">Image Compressor</h1>
            <p className="mb-2 text-lg text-text-muted">Compress JPG, PNG and WebP images in batch with quality control. Download as ZIP. 100% in your browser.</p>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
            { icon: "🗜️", title: "Batch compression", desc: "Compress many images at once with a quality slider and download them all in a ZIP." },
            { icon: "🔒", title: "100% private", desc: "Everything runs in your browser with the Canvas API. Your images never leave your device." },
            { icon: "🔄", title: "Format conversion", desc: "Optionally convert to JPEG, WebP or PNG while compressing." },
            ].map((item) => (
          ...
          <div className="mt-8 rounded-xl border border-border/20 bg-white/3 p-6">
            ...
            <p className="mb-4 text-sm text-text-muted">Add Image Compressor to any page with a simple iframe, or link to it with attribution.</p>
            <div className="mb-3 rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Iframe (plug & play):</p>
              <code className="text-xs text-green-400 break-all">{`<iframe src="${EMBED_URL}" width="100%" height="700" style="border:none;border-radius:12px;" title="Image Compressor — miguelacm.es" loading="lazy"></iframe>`}</code>
            </div>
            <div className="rounded-lg bg-black/40 p-3">
              <p className="mb-1 text-xs text-text-muted/60">Link with attribution (recommended for backlink):</p>
              <code className="text-xs text-green-400 break-all">{`<a href="${SITE_URL}" target="_blank" rel="noopener">Image Compressor — free tool by MACM</a>`}</code>
            </div>
          </div>
        </div>
      </main>
